...
describe('Planning permission page', () => {
  ...
  it('if value = \'Will not be in place by 31 January 2023\' ==> disqualify user', async () => {
    const postOptions = {
      method: 'POST',
      url: `${global.__URLPREFIX__}/planning-permission`,
      payload: { planningPermission: 'Will not be in place by 31 January 2023', crumb: crumbToken },
      headers: { cookie: 'crumb=' + crumbToken }
    }
  ...
  it('if value = \'Should be in place by 31 January 2023\' ==> store and redirect to planning-required-condition page', async () => {
    const postOptions = {
      method: 'POST',
      url: `${global.__URLPREFIX__}/planning-permission`,
