      ...
      title: title.replace(
        ...
        (_ignore, additionalYarKeyName) => {
          if (additionalYarKeyName === "currentlyIrrigating") {
            return getYarValue(request, additionalYarKeyName) === "Yes"
              ? SUMMER_ABSTRACTION_MAINS_YES
              : SUMMER_ABSTRACTION_MAINS_NO;
          }
...
const showPostPage = (currentQuestion, request, h) => {
    ...
    allFields.forEach((field) => {
      ...
      dataObject = {
        ...
        [field.yarKey]:
          ...
          field.yarKey === "projectPostcode" ||
          field.yarKey === "businessPostcode"
            ? payloadYarVal
            : payload[field.yarKey] || "",
  ...
  if (title === "Will you {{_currentlyIrrigating_}} summer abstraction or mains?") {
    currentQuestion = {
      ...
      validate: [
        {
          type: "NOT_EMPTY",
          error: currentQuestion.validate[0].error.replace(
            SELECT_VARIABLE_TO_REPLACE,
            (_ignore, additionalYarKeyName) => {
              if (additionalYarKeyName === "currentlyIrrigating") {
                return getYarValue(request, additionalYarKeyName) === "Yes"
  ...
  if (
    ...
    if (thisAnswer?.alsoMaybeEligible) {
      const { maybeEligibleContent } = thisAnswer.alsoMaybeEligible;
  ...
  if (yarKey === "projectCost") {
    const { calculatedGrant, remainingCost, projectCost } = getGrantValues(
      payload[Object.keys(payload)[0]],
      currentQuestion.grantInfo
    );
